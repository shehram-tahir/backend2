...
def estimate_api_calls(categories_data, included_categories, excluded_categories):
    ...
    included_calls = sum(categories_data[cat] for cat in included_categories if cat in categories_data)
    excluded_calls = sum(categories_data[cat] for cat in excluded_categories if cat in categories_data)
    total_calls = included_calls - excluded_calls
    return max(min_calls, total_calls) * MAX_CALLS_PER_CITY


async def calculate_cost(req: ReqCostEstimate):
    # Load city info from appropriate json file
    file_path = f"Backend/country_info/{req.country_name.lower()}/city_info/{req.city_name.lower()}/ggl_categories.json"
    categories_data = await use_json(file_path, "r")

    # Flatten the nested dictionary
    flattened_categories = {subcat: value for cat in categories_data for subcat, value in categories_data[cat].items()}

    api_calls = estimate_api_calls(flattened_categories, req.included_categories, req.excluded_categories)
